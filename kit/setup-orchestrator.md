...
- **OAuth must complete BEFORE running `first-time-setup.sh`.** The provisioner script has a pre-flight gate that checks for `$HOME/.claude/.credentials.json` and aborts (with a clear error in `--non-interactive` mode) if it's missing. The order is: bootstrap → switch to bot user → walk `claude` OAuth → THEN run the env-var-driven `first-time-setup.sh --non-interactive`. Earlier versions of this checklist implied the OAuth walk could happen after the script; that's wrong, and a fresh-LXC walk on 2026-05-12 (F35) confirmed the gate trips at script line 174. The pre-flight order also matters operationally: step 2 of the script renders the systemd unit that immediately launches `claude` in tmux, which crashloops if credentials are missing.
