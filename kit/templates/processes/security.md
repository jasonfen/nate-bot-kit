...
**Note on `runtime/sb-cmd.sh`.** The kit ships a runtime wrapper at `<KIT>/runtime/sb-cmd.sh` that invokes SilverBullet's `POST /.runtime/lua` endpoint. It pre-dates this doctrine and currently uses the hand-rolled-wrapper pattern (decrypts the token, places it in a `curl -H` header). It is **operator-tier** — for ad-hoc programmatic SB command invocation from a human terminal session — and **not** for agent-side use. The bot's runtime should not invoke `sb-cmd.sh` from agent code. Future kit versions will fold `sb-cmd.sh` into a broker-pattern equivalent when (and only when) a real bot-as-client driver justifies building the broker; deprecating it sooner pushes the same wrapper pattern into operators' ad-hoc scripts with a worse audit surface.

> **Threat-model footnote.** The argv leak from `sb-cmd.sh` is real but bounded on this kit's intended deployment model: single-tenant box (one bot, one operator), Tailscale-isolated (no internet-side attackers), `/proc/<pid>/cmdline` readable only by processes in the same uid (the bot user, who is also the operator). Practical exposure on a target kit deployment is near-zero. The eventual broker-wrap is a **doctrinal cleanup**, not a security patch to apply urgently. If the kit is ever adapted to multi-tenant or non-Tailscale-isolated boxes, the urgency increases.
