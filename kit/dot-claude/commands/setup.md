...
Normally the soul-loop calls setup-runner automatically when `<REPO_ROOT>/setup-state.md` Current phase != done. Use this command to:
...
grep -E '^(Current phase|## (Done|In-progress|Blockers))' <REPO_ROOT>/setup-state.md 2>/dev/null || echo "(no setup-state.md — bot is already done)"
```

If `Current phase: done` and you still want to re-run a specific phase, edit `<REPO_ROOT>/setup-state.md` to set Current phase to the phase name (e.g., `step-5-cron`) before invoking this command.
